...
export function CollegeCard({ match, studentPrefs, rank }: Props) {
  ...
  // Keep both polygons on the same normalized 0–10 scale so they're directly comparable.
  // - Student sliders are already 1–10.
  // - Backend match dimensions are usually 0–1; we detect and scale accordingly.
  const neutralVal = 5;
  const clamp01to10 = (v: number) => Math.max(0, Math.min(10, v));
  const normalizeCollegeDim = (raw: unknown) => {
    const n = typeof raw === "number" ? raw : Number(raw);
    if (!Number.isFinite(n)) return 0;
    // If the backend is returning 0..1, scale to 0..10. If it's already 0..10, keep as-is.
  ...
  const studentVals = EXPERIENCE_DIMS.map((d) => {
    ...
    // Sliders should be 1–10; treat 0/unset/invalid as "not rated" -> neutral.
  ...
  // Tie the axis markers to existing tag semantics:
              ...
              >
                {[1, 2, 3, 4, 5].map((n) => (
                  <Star
                    ...
                    size={11}
                    fill={
                      n <= Math.round(avgRating ?? 0) ? "currentColor" : "none"
                    }
